...
// Add a new netCDF scalar variable.
NcVar NcGroup::addVar(const string& name, const NcType& ncType) const {

	// get an NcType object with the given type name.	
	if (ncType.isNull()) throw NcNullType("Attempt to invoke NcGroup::addVar failed: typeName must be defined in either the current group or a parent group", __FILE__, __LINE__);

	// finally define a new netCDF  variable
	int varId;	
	ncCheck(nc_def_var(myId, name.c_str(), ncType.getId(), 0, NULL, &varId), __FILE__, __LINE__);
